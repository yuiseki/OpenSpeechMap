...
![ビューア。地名が言われた場所と、時系列上の異常な日](docs/viewer.png)

同梱のサンプルで、日本の公共ラジオ 57 日分である。2026-07-28 のスパイクは熊本の地震で、マーカーが九州に集まっているのもそれが理由。赤い帯はスコアが閾値を超えた日、紫の線は跳ねたのではなく水準が動いた場所を指す。自分で動かせば、自分のレンズと自分のストリームがここに入る。
